...
### Requirement: Hardcoded container type constants

The hardcoded `ContainerType` constants (`ContainerTypeBase`, `ContainerTypeDev`, `ContainerTypeOsint`) SHALL be removed. Container types are now defined in configuration.

**Reason:** Config-driven approach allows user customization.

#### Scenario: No hardcoded type validation

- Given code previously called `IsValidContainerType("osint")`
- When the change is applied
- Then validation uses `ImageManager.IsValidType()` instead

### Requirement: Local image building removed

The server SHALL NOT build container images locally. The `ContainerTypeBuilder` is removed and `image_rebuild` MCP tool is replaced with `container_refresh`.

**Reason:** End users don't have Go toolchain or source code needed to build images. Building is dev-only via `build.sh`.

#### Scenario: EnsureImageExists pulls instead of builds

- Given image does not exist locally
- When `EnsureImageExists` is called
- Then `Pull` is called (not `Build`)

#### Scenario: image_rebuild tool removed

- Given the MCP tools registry
- When tools are listed
- Then `image_rebuild` does not exist
- And `container_refresh` exists instead

### Requirement: ImageNameForType function

The `ImageNameForType()` function SHALL be removed. Image names are resolved via `ImageManager.GetImageName()`.

**Reason:** Image names come from config, not hardcoded format strings.
