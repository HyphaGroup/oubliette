...
- [ ] Update `test/pkg/suites/project.go` for config-driven container types
- [ ] Add integration test for image pull
- [ ] Test full install flow: `install.sh` → `init` → `oubliette` with image pull
- [ ] Verify both Docker and Apple Container runtimes pull correctly
