...
## Purpose
Enable agents inside containers to reach external MCP servers on the host via HTTP proxy relay.
...
#### Scenario: Configure proxy for external MCP server
- **GIVEN** a client calling `session_message`
- **WHEN** the context includes `http_proxies.myapp` with URL and headers
...
- **GIVEN** an active session with proxy config for target "myapp"
- **WHEN** oubliette-client sends an `http_proxy` request with target "myapp"
...
- **GIVEN** the HTTP proxy server is running with target "myapp" configured
- **WHEN** the agent makes a request to `http://127.0.0.1:19999/myapp/mcp`
- **THEN** the request is forwarded with path `/mcp` to the "myapp" target
